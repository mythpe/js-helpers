...
var ParseArabicNumber = function ParseArabicNumber(n) {
  if (!n) return n;
  n = n.toString();
  var en, ar;
...
var FromNumberFormat = function FromNumberFormat(n) {
  if (!n) return n;
...
var ToNumberFormat = function ToNumberFormat(num, currency) {
  if (!num) return num;
...
var IsKsaMobile = function IsKsaMobile(mobile) {
...
var ParseFormData = function ParseFormData(form_data, values, name) {
...
var DownloadFileFromResponse = function DownloadFileFromResponse(response) {
  if (!response) return;
  ...
  try {
    ...
    if (!fileLink_1 || !fileURL_1) return;
...
var SortNumberFormatted = function SortNumberFormatted(a, b) {
  var f = parseFloat(FromNumberFormat(a).replace(/[^\d]/g, '')) || 0;
  var s = parseFloat(FromNumberFormat(b).replace(/[^\d]/g, '')) || 0;
  return f - s;
};

exports.SortNumberFormatted = SortNumberFormatted; // import _ from 'lodash'
